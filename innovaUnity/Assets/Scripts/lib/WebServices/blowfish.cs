...
namespace BlowFishCS
{
    class BlowFish
    {
        ...
        public string Encrypt_CBC(string pt)
        {
            if (!IVSet)
                SetRandomIV();
            return ByteToHex(InitVector) + ByteToHex(Encrypt_CBC(Encoding.UTF8.GetBytes(pt)));
        }
        ...
        public string Decrypt_CBC(string ct)
        {
            ...
            return Encoding.UTF8.GetString(Decrypt_CBC(HexToByte(ct.Substring(16)))).Replace("\0", "");
        }
        ...
        /// </summary>
        /// <param name="pt">Plaintext to encrypt as UTF8 string</param>
        /// <returns>hex value of encrypted data</returns>
        public string Encrypt_ECB(string pt)
        {
            return ByteToHex(Encrypt_ECB(Encoding.UTF8.GetBytes(pt)));
        }
        ...
        /// <returns>Plaintext UTF8 string</returns>
        public string Decrypt_ECB(string ct)
        {
            return Encoding.UTF8.GetString(Decrypt_ECB(HexToByte(ct))).Replace("\0","");
        }
